...
class FootballSpider(scrapy.Spider):
    ...
    def parse_club(self, response):
        ...
        #     player_name = player.xpath('//tr[th/a/text()="FullName"]/td').get()
        #     player_position = player.xpath('//tr[th/a/text()="Position(s)"]/td').get()
        #     player_age = player.xpath('//tr[th/a/text()="Date of birth"]/td').get()
        #     player_data = {
        #         'name': player_name,
        ...
        #     json.dump(club_players, f, indent=4)
            yield response.follow(player_url, self.parse_club, meta={'club_name': club_name, 'league_name': league_name})
    
    def parse_player(self, response):
        listingUrls = []
        for item in response.xpath(x):
        ...
        if len(listingUrls) > 0:
            for url in listingUrls:
                yield Request(url=url, callback=self.parse_player_data, meta={'listing_url': url})
                time.sleep(5)
